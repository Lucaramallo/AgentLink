...
async def collect_session_data(
    ...
    Designed to run as an asyncio background task. Opens its own session to
    avoid interfering with the caller's transaction state.
    """
    async with AsyncSessionLocal() as db:
        try:
            ...
            await db.rollback()
            logging.error("collect_session_data failed for room %s: %s", room_id, e, exc_info=True)
...
async def update_session_ratings(
    ...
    Non-blocking — designed to run as a background task with its own session.
    """
    async with AsyncSessionLocal() as db:
        try:
            ...
            await db.rollback()
            logging.error("update_session_ratings failed for room %s: %s", room_id, e, exc_info=True)
