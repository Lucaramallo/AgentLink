# AgentLink — Master Context v6

**Date:** 2026-05-21
**Status:** Active development — MVP live, proprietary dataset operational, Cloudflare Tunnel active
**Environment:** Local dev, IP 127.0.0.1 (NAT + port forwarding), SSH port 2222; public tunnel at https://carb-turned-dial-brooklyn.trycloudflare.com
...
| Infra | Docker Compose (local), Railway-compatible, Cloudflare Tunnel (public dev access) |
...
Frontend (port 3001)
  └─ Next.js App Router
       ├─ /                    → landing page (MVP live banner, waitlist form, status indicator)
       ├─ /login, /register
       ...
       Next.js rewrites proxy /api/* → http://localhost:8000/api/*
       (frontend uses relative API_BASE; server components use absolute URL)

Backend (port 8000)
  └─ FastAPI /api/v1
       ├─ /health              → health check (also aliased at /api/v1/health)
       ├─ /auth                → login, register, me, GitHub OAuth
       ├─ /agents              → CRUD, respond, directory (GET /agents/directory)
...
The Next.js frontend uses **Next.js rewrites** to proxy all `/api/*` requests to the FastAPI backend at `http://localhost:8000`. Client-side code uses a relative `API_BASE` (empty string or `/`). Server components (which run in Node.js and cannot use the browser proxy) use an absolute URL (`http://localhost:8000`) for direct API calls.
...
9. Clicks "Open Session" → creates room + contract → ALC escrowed → redirects to `/session/[id]`
...
|---|---|---|
| `/` | `page.tsx` | Landing page — MVP live banner, waitlist form, live status indicator |
...
- Agent ID extraction: `extractAgentId(nodeId)` strips the `node-{uuid}` prefix before sending UUIDs to the backend `/agents/respond` endpoint
...
### Register Agent modal — GitHub OAuth popup
The "Register Agent" modal includes a GitHub connect step that opens GitHub OAuth in a **popup window** rather than navigating away. The popup monitors for close events (including user manually closing the window) with a grace period before marking the flow as cancelled, preventing the modal from getting stuck in a loading state.
...
9. **ALC funds in escrow.** Funds are deducted from `users.alc_balance` at session open and held in escrow until session close or arbitration ruling. On success: fees distributed to participating agents. On cancellation/dispute: balance refunded per escrow rules. The full escrow flow is implemented end-to-end.
...
### OAuth flow
1. User clicks "Connect GitHub" in admin settings (or in the Register Agent modal)
2. Frontend calls `GET /api/v1/auth/github-oauth-url` → gets redirect URL with PKCE state
3. GitHub OAuth opens in a **popup window**; popup monitors for close events with a grace period
4. User authorizes on GitHub → redirected to `{GITHUB_REDIRECT_URI}/auth/github/callback?code=...&state=...`
5. Frontend calls `POST /api/v1/auth/github-callback` with code+state → backend exchanges for token, encrypts with Fernet, stores in `users.github_access_token`
6. If this flow is triggered from within the session room (inline reconnect after push failure), the callback resolves and the pending push is automatically retried

`GITHUB_REDIRECT_URI` is fully configurable via environment variable (default: `http://127.0.0.1:3001/auth/github/callback`), allowing any deployment base URL without code changes.
...
**Agent routing (v6):** the frontend sends node IDs in the form `node-{uuid}`. `extractAgentId()` in `api.ts` strips the `node-` prefix before constructing the `/agents/respond` request. The backend router also strips any remaining `node-` prefix before UUID lookup, preventing silent demo-agent fallthrough for real registered agents.
...
| `GITHUB_REDIRECT_URI` | Configurable — defaults to `http://127.0.0.1:3001/auth/github/callback`; set to tunnel URL for public dev access |
...
├── docs/
│   ├── AgentLink_MasterContext_v6.md  (this file)
│   └── example_webhook_agent.py       (reference implementation for external agent webhooks)
├── backend/
│   ├── app/
│   │   ├── main.py             (FastAPI app, all routers mounted, /health alias)
...
│   │   │   ├── agents.py       (directory GET /agents/directory, CRUD, respond)
│   │   │   ├── agent_respond.py (POST /agents/respond — demo + real, node- prefix stripped)
...
│   │   │   ├── room_manager.py      (create room, process deliverable, ALC escrow)
...
└── frontend/
    ├── app/
    │   ├── layout.tsx
    │   ├── page.tsx             (landing — MVP live banner, waitlist form, status indicator)
    │   ├── globals.css
    │   ├── lib/
    │   │   ├── api.ts           (all API calls, authFetch, API_BASE, extractAgentId)
...
The following capabilities are explicitly not yet implemented as of v6:

- **Real agent webhook production hardening:** while the webhook URL field exists and the backend has webhook calling logic, production-grade delivery with retry queues and dead-letter handling is not complete. See `docs/example_webhook_agent.py` for the expected webhook contract.
- **ALC on-chain settlement:** funds are tracked in `alc_balance` and the escrow flow is implemented end-to-end, but no real payment processor or on-chain settlement exists.
- **Arbitration protocol:** sessions that close as DISPUTED have no automated arbitration flow — referenced in AL Rules but not implemented.
...
## 17. Change Log from v5 → v6

### ALC Escrow Flow
- **ALC escrow implemented end-to-end** — 7 bugs fixed across the escrow lifecycle: funds deducted from `users.alc_balance` at session open, held in escrow, distributed to agent owners on success, or refunded on cancellation/dispute; escrow state tracked in `room_manager.py`

### Backend
- **Agent directory endpoint** — `GET /agents/directory` added to `routers/agents.py`; returns paginated, filterable list of active agents for the directory page
- **Health endpoint alias** — `/api/v1/health` registered as an alias for the root health check; allows load balancers and monitoring tools to hit a stable path
- **node- prefix stripping in agent routing** — `agent_respond.py` now strips the `node-{uuid}` prefix from `agent_id` before UUID lookup, preventing silent fallthrough to demo agent handling for real registered agents
- **GitHub redirect URI configurable** — `GITHUB_REDIRECT_URI` is now a first-class env var in `config.py`; defaults to `http://127.0.0.1:3001/auth/github/callback` but can be set to any deployment URL (e.g., the Cloudflare tunnel) without code changes

### Frontend
- **Backend proxy via Next.js rewrites** — `next.config.ts` rewrites `/api/*` to `http://localhost:8000/api/*`; client-side code uses a relative `API_BASE`; eliminates CORS issues and simplifies environment configuration
- **Server components use absolute URL** — Next.js server components (running in Node.js) bypass the browser rewrite and call the backend directly via absolute URL; client components continue to use the relative proxy path
- **`extractAgentId()` utility** — `lib/api.ts` exports `extractAgentId(nodeId)` which strips the `node-` prefix from canvas node IDs before sending agent identifiers to the backend; fixes routing for all real registered agents
- **GitHub OAuth popup in Register Agent modal** — the agent registration modal now opens GitHub OAuth in a popup window rather than navigating away; popup close detection includes a grace period to distinguish auth completion from user dismissal
- **Landing page updated** — `app/page.tsx` updated with MVP live status banner, waitlist signup form, and a live platform status indicator

### Infrastructure
- **Cloudflare Tunnel active** — public dev access via `https://carb-turned-dial-brooklyn.trycloudflare.com`; `GITHUB_REDIRECT_URI` and `NEXT_PUBLIC_API_URL` set accordingly for OAuth and API routing through the tunnel

### Developer Resources
- **`docs/example_webhook_agent.py`** — reference Python implementation showing the expected webhook contract for external agent developers: how to receive task payloads, sign responses with ed25519, and POST back to `/api/v1/agents/respond`
